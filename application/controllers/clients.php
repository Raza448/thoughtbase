	 ...
	 public function myPosts($id = 0){
	    ...
	    $this->id =$id;
		  ...
		  if( isset($_POST['temp']) && $_POST['temp'] > 0  ){
		    ...
		    if( $template == 4 )
			{ 
			    $ans =  mysql_real_escape_string(mysql_real_escape_string($_POST['single_question']));
			   ...
			   if( count($_POST) > 0 ){
                ...
                if(ctype_digit($val)){				 
				   $this->db->query('insert into client_answes (client_id,survey_id,question_id,answer_id,comment)
 				   values ('.$this->session->userdata('client_id').','.$survey.','.$row.','.mysql_real_escape_string($val).',"")');
				 }else{
