	...
	public function delimg($id=0){
	 ...
	 public function deactivate(){
			...
			if($this->session->userdata('emails')) {
				...
				//$httpParsedResponseAr = $this->PPHttpPost('DoDirectPayment', $nvpStr);
					...
					if(count($cout) > 0){
					 ...
					 echo '<pre>';
					 echo $bodyOfMessage = $this->load->view( 'survey_message', array('name'=>$name, 'survey_name'=>$survey_name, 'p_amount'=>$p_amount, 'id'=>$kavl['uid']), true );
					 echo '</pre>';
	  ...
	  if((0 == sizeof($httpParsedResponseAr)) || !array_key_exists('ACK', $httpParsedResponseAr)) {
		...
		exit("There was an error processing your payment <a href='".site_url('business/step4')."'>Click here</a>");
