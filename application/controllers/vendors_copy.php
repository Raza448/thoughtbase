			...
			if( $this->input->post('act') == 'editaccount' ){
				...
				if($this->form_validation->run() == FALSE){
			      ...
			      //check this email address exists
				   $res = $this->vendor->emailDuplication($this->session->userdata('vendor_id'),$this->input->post('email'));
				  if(count($res) > 0 ){
	 ...
	 public function deactivate(){
	  ...
	  public function create(){
		...
		//form validation failed   
		   ...
		   if($this->session->userdata('survey_id')){
			 ...
			 $this->db->query('update survey_temp set name= "'.$this->input->post('title').'",description ="'.$this->input->post('description').'" where id ='.$survey_id);
		   }else{
		     $this->db->query('insert into survey_temp (id,name,description,user_id) values(null,"'.$this->input->post('title').'","'.$this->input->post('description').'",'.$this->session->userdata('vendor_id').')');
	    ...
	    $this->layout->view('vendor_only/create1');
	  }
	  ...
	  public function create_step2(){
	    if($this->session->userdata('step1')){
		  ...
		  if( $_POST['template']  == 4 ){
		    $this->db->query('update survey_temp set template= "'.$this->input->post('template').'" , question="'.$this->input->post('question').'" where id ='.$this->session->userdata('survey_id'));
		  ...
		  if($this->form_validation->run() == FALSE){
		    ...
		    $this->db->query('update survey_temp set template= "'.$this->input->post('template').'"   where id ='.$this->session->userdata('survey_id'));
	     ...
	     if($this->session->userdata('step2')){
		   ...
		   if(isset($_POST['step3']) && $_POST['step3'] == 'step3' ){
			...
			if( isset($_POST['interest']) && count($_POST['interest']) > 0 ){
             ...
             $this->session->set_userdata(array('step3'=>'1'));  
	   ...
	   public function create_step4(){
	     ...
	     if($this->session->userdata('step3')){
          ...
          if(isset($_POST['step4']) && $_POST['step4'] == 'step4' ){
    	   ...
    	   $this->layout->title('Create Query - Step 4');   	   
