	 ...
	 public function myQueries($id = 0){
	    ...
	    $this->id =$id;
		  ...
		  if( isset($_POST['temp']) && $_POST['temp'] > 0 ){
			...
			$check_survey = $this->db->query('select * from clients_survey where survey_id ='.$survey.' and user_id='.$this->session->userdata('client_id').'')->row_array();
			if($check_survey['completed'] == 0){ 
			...
			if ($ack == "Success"){
			   //update user earnings
				...
				//print_r($response);
				$this->session->set_flashdata('Error','Query could not be submitted. Please try again.');
				redirect('user/myQueries');
				exit;
			}
		  }
