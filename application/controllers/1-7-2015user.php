			   ...
			   if(count($_POST['interest']) > 0){
				...
				if($this->form_validation->run() == FALSE){
				  ...
				  if(count($res) > 0 ){
					...
					$this->client->updateClientUser($this->session->userdata('client_id'),array('age'=>mysql_real_escape_string($this->input->post('age')),'email'=>mysql_real_escape_string($this->input->post('email')),'username'=>mysql_real_escape_string($this->input->post('username')),'password'=>mysql_real_escape_string(base64_encode($this->input->post('password'))),'token'=>'','gender'=>mysql_real_escape_string($this->input->post('gender')),'profile'=>'1','zipcode'=>mysql_real_escape_string($this->input->post('zipcode'))));
	 ...
	 public function myQueries($id = 0){
	    ...
	    $this->id =$id;
		  ...
		  if( isset($_POST['temp']) && $_POST['temp'] > 0 ){
			...
			$ppuserid = $this->getMetaValue("api_username");
			$pppass   = $this->getMetaValue("api_password");
			$ppsig    = $this->getMetaValue("api_signature");
			$ppappid  = $this->getMetaValue("app_id");
			...
			$payLoad["sender"]["email"] = $this->getMetaValue("payment_email");//the paypal email address of the where the money is coming from
			//run the call
