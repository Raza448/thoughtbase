...
<?$this->layout->block()?>
	<div class="container">
		...
		<?php
			if($result['customize'] == 1){
				$result2 = $this->db->query('select * from custom_questions where template_id ='.$result['template'].' and question_cat=3 order by id asc')->result_array();
			}else{
			...
			<div class="col-md-12 old" style="padding:0;">
				...
				<ul class="portfolio-list" data-sort-id="portfolio" style="position: relative;">
						  ...
						  <div class="col-md-9" style="padding-bottom:10px;" >
							...
							<?php
							if($result['customize'] == 1){
								$result1 = $this->db->query('select * from custom_questions where template_id ='.$result['template'].' and survey_id ='.$result['id'].' order by id')->result_array();
								//print_r('select * from custom_questions where template_id ='.$result['template'].' and survey_id ='.$result['id'].' order by question_cat');
							}else{
...
<?php
if($result['customize'] == 1){
	$sql = 'select custom_questions.question_cat,client_answes.id as kid,client_answes.comment,client_answes.client_id,client_answes.survey_id,client_answes.question_id, users.age,users.gender,users.zipcode from client_answes left join custom_questions on client_answes.question_id = custom_questions.id left join users on client_answes.client_id = users.id where client_answes.survey_id='.$result['id'].' and client_answes.comment !=""';
}else{
		...
		<div class="col-md-12 new" style="padding:0;display:none;">
				...
				<ul class="portfolio-list" data-sort-id="portfolio" style="position: relative;">
						  ...
						  <div class="col-md-9" style="padding-bottom:10px;" >
							...
							<?php
							 if($result['customize'] == 1){
								$result1 = $this->db->query('select * from custom_questions where template_id ='.$result['template'].' and survey_id ='.$result['id'].' order by question_cat')->result_array();
							}else{
								$result1 = $this->db->query('select * from survey_questions where survey_id ='.$result['template'].' order by question_cat')->result_array();
							}
							?>
...
<?php
if($result['customize'] == 1){
	$sql = 'select custom_questions.question_cat,client_answes.id as kid,client_answes.comment,client_answes.client_id,client_answes.survey_id,client_answes.question_id, users.age,users.gender,users.zipcode from client_answes left join custom_questions on client_answes.question_id = custom_questions.id left join users on client_answes.client_id = users.id where client_answes.survey_id='.$result['id'].' and client_answes.comment !="" and question_cat=2';
}else{
...
<script type="text/javascript">
   function  callmanu(){
	...
	//alert(question);
	var sort = 1;
	var customize = "<?php echo $result['customize'] ?>";
	var a = $('.old #sort').val();
	var b = $('.new #sort').val();
	if(a == null){
		var sort = b;
	...
	if( question > 0 ){
		$.ajax({ 
		  ...
		  dataType : 'json',
		  async : false,
		  data: {question : question ,sort: sort,survey:survey, customize:customize}, 
