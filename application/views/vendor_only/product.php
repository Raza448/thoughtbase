...
class ModelCatalogProduct extends Model {
	...
	* By: Piyush	
	*/
	public function getTotalActiveProductCount(){
		$count_sql = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_option_value pov on pov.product_id=p.product_id and option_id in (5,11) and p.status=1");              
                return $count_sql->row['total']; 
	}
	...
	* By: Piyush	
	*/
	public function getActiveProducts($data = array()) {
		$page_num = (int)$data['page_num'];
		$page_size = (int)$data['page_size'];
		$start_limit = $page_size * ($page_num - 1);
		$option_id = $data['option_id'];
		$product_sql = $this->db->query("SELECT p.product_id, p.model, pov.option_value_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_option_value pov on pov.product_id=p.product_id and option_id in (".$option_id.") and p.status=1 LIMIT ".$start_limit.",".$page_size.";"); 
		return $product_sql->rows;
	}
	...
	*/
	public function getProductDetail($data = array()) {
		...
		$product_image_sql = $this->db->query("select image from ". DB_PREFIX ."product_image where product_id=".$product_id.";");
                $images = $product_image_sql->rows;

		if($option_value_id == 0)
		{
			$product_detail_sql = $this->db->query("SELECT p.product_id,p.model,p.sku,pd.name FROM ". DB_PREFIX ."product p, " . DB_PREFIX ."product_description pd where p.product_id=".$product_id." and p.product_id=pd.product_id;");

                $product_detail = array(
                        'productId' => $product_detail_sql->row['product_id'],
                        'model' => $product_detail_sql->row['model'],
                        'sku' => $product_detail_sql->row['sku'],
                        'name' => $product_detail_sql->row['name'],
                        'imageUrls' => $images
                );
		}
		else
		{		
		$product_detail_sql = $this->db->query("SELECT p.product_id,p.model,p.sku,pd.name,ovd.name AS variant_name FROM ". DB_PREFIX ."product p, " . DB_PREFIX ."product_description pd, " . DB_PREFIX ."option_value_description ovd where p.product_id=".$product_id." and ovd.option_value_id=".$option_value_id." and p.product_id=pd.product_id;");
		...
		$product_detail = array(
			...
			'sku' => $product_detail_sql->row['sku'],
			'name' => $product_detail_sql->row['name'],
			'variantName' => $product_detail_sql->row['variant_name'],
			'imageUrls' => $images
		);
		}
	...
	*/
	public function getVariantName($option_id){
		$option_value_id = (int)$data['option_id'];
		$variant_sql = $this->db->query("SELECT name AS variant_name FROM ". DB_PREFIX ."option_value_description where option_value_id=".$option_value_id.";");
		return $variant_sql->row['variant_name'];
	}
	...
	****/
	public function editProduct($data) {
		...
		{
			$this->db->query("UPDATE " . DB_PREFIX ."product_option_value SET quantity ='".(int)$quantity."' WHERE product_id =".(int)$product_id." and option_value_id=".$option_id.";");
			$count_sql =   $this->db->query("SELECT SUM(quantity) AS count FROM " . DB_PREFIX ."product_option_value WHERE product_id=".$product_id.";");
			$count = $count_sql->row['count'];
			
			$this->db->query("UPDATE " . DB_PREFIX . "product SET quantity = '" . (int)$count. "', date_modified = NOW() WHERE product_id = '" . (int)$product_id . "'");
		}
	}
